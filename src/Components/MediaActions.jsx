  ...
  const sendToMerge = async (e) => {
    ...
    if (!!files?.length) {
      try {
        ...
        data.append("audio", JSON.stringify(selectedAudio));
        data.append("star", `star${rating}`);

        const resp = await getImageMerged(data);
        if (resp.type === "video") setIsVideo(true);
  ...
  const shareBtn = () => {
    if ("share" in navigator) {
      navigator
        ...
        .catch((error) => {
          setError(
            error.message ? error.message : "Error al compartir archivo",
          );
        });
    } else {
    ...
    <>
      <div className={`grid grid-cols-${colSize ? "2" : "1"} gap-4 mt-2`}>
        ...
        <div className="flex justify-center">
          <Button handleOnClick={onPhotoBtnClick}>
        ...
        {selectedImg && (
          <div className="flex justify-center">
            <Button role="button" handleOnClick={shareBtn}>
