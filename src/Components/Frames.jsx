...
import { setAssetsUrl } from "../services/http";
// otros
...
const Frames = () => {
  const { assets, selectedImg, setSelectedFrame, selectedAudio } =
    useContext(MediaContext);

  const img_url = setAssetsUrl();

  const [url, setUrl] = useState(noImage);
  const [focus, setFocus] = useState();

  const handleUrl = (selected) => {
    const _url = `${img_url}/${selected}`;
    setUrl(_url);
    setFocus(selected);
  };
  ...
  return (
    <div className="grid gap-4 h-auto my-2">
      {!selectedImg && (
        <>
          <h1 className="text-center text-2xl text-orange-500">{titulo}</h1>
      ...
      {!selectedAudio && (
        <>
          <section className="flex overflow-x-auto space-x-8 w-1/1">
            {assets.frames.map((frame) => (
              ...
              >
                <img
                  onClick={() => {
                    handleUrl(frame.name);
                  }}
                  src={`${img_url}/${frame.name}`}
          ...
          <Button disabled={!focus} handleOnClick={selectFrame}>
